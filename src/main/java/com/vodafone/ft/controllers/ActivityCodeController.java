...
public class ActivityCodeController implements Serializable {
    ...
    @EJB
    private com.vodafone.ft.beans.ActivityCodeFacade ejbFacade;
    ...
    public List<ActivityCode> autoCompleteActivityCode(String query){
        List<ActivityCode> allSites = getItems();
        List<ActivityCode> filteredSites = new ArrayList<>();
        query = query.toLowerCase();
        for (int i = 0; i < allSites.size(); i++) {
            ActivityCode code = allSites.get(i);
            if(code.getId().toLowerCase().contains(query) || code.getDescription().toLowerCase().contains(query)) {
